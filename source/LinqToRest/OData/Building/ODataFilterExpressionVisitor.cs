...
namespace LinqToRest.OData.Building
{
	public class ODataFilterExpressionVisitor : ExpressionVisitor
	{
		...
		private readonly IODataFilterExpressionBuilderStrategy _expressionBuilderStrategy;
		private readonly Stack<string> _expression = new Stack<string>();

		public ODataFilterExpressionVisitor() : this(DependencyResolver.Current.GetInstance<IODataFilterExpressionBuilderStrategy>())
		{
		}

		public ODataFilterExpressionVisitor(IODataFilterExpressionBuilderStrategy expressionBuilderStrategy)
		{
			_expressionBuilderStrategy = expressionBuilderStrategy;
		}

		public ODataQueryFilterExpression Translate(Expression expression)
		{
			Visit(expression);

			var expr = _expressionBuilderStrategy.BuildExpression(_expression);
		...
		protected override Expression VisitBinary(BinaryExpression node)
		{
			...
			if ((op == ODataQueryFilterExpressionOperator.Add) && (node.Left.Type == typeof(string)))
			{
		...
		protected override Expression VisitUnary(UnaryExpression node)
		{
			...
			switch (node.NodeType)
			{
				...
				case ExpressionType.Negate:
					_expression.Push(ODataQueryFilterExpressionOperator.Negate.GetODataQueryOperatorString());
					break;
				case ExpressionType.NegateChecked:
					_expression.Push(ODataQueryFilterExpressionOperator.Negate.GetODataQueryOperatorString());
					break;
				case ExpressionType.Not:
					_expression.Push(ODataQueryFilterExpressionOperator.Not.GetODataQueryOperatorString());
					break;
				case ExpressionType.Increment:
					_expression.Push("1");
					_expression.Push(ODataQueryFilterExpressionOperator.Add.GetODataQueryOperatorString());
					break;
				case ExpressionType.Decrement:
					_expression.Push("1");
					_expression.Push(ODataQueryFilterExpressionOperator.Subtract.GetODataQueryOperatorString());
					break;
